...
const {isLocalUrl, isFileExists, getFileTokens, findBlockTokens, headingInfo, isExistId} = require('../utils');
...
function processLink(state, tokens, idx, opts) {
    ...
    if (hash) {
        ...
        if (!isIncludedFile && !isExistId({file, id, state, opts, root, fileExists, isPageFile})) {
            log.error(`Link is unreachable, hash does not exist: ${bold(href)} in ${bold(currentPath)}`);
        }
    }
