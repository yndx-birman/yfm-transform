...
const isExistId = ({file, id, state, opts, isPageFile, fileExists}) => {
    ...
    const hasInCache = Boolean(docsIds[file] && docsIds[file][id]);

    if (!hasInCache) {
        state.md.enable(['includes']);
        state.env.includes = [];
        const fileTokens = getFileTokens(file, state, opts);
        state.md.disable(['includes']);
